...
float azimuth = 0.6, elevacion = 0.6, D=3.0f;
int mx0=0,my0=0;
...
void render_scene()
{
	...
	pos_obs = vec3(D*cos(azimuth)*cos(elevacion), D*sin(azimuth)*cos(elevacion), D*sin(elevacion));
...
//	glutSpecialFunc(key_special);  // Teclas de funci�n, cursores, etc
	...
	glutMouseFunc(mouse_click);            // Eventos del rat�n
